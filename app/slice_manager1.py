...
def main():
    ...
    ### FOR SLICE 2 ### uncomment if more than 1 slice
    ###if (True):
        # We put the IP of the machine executing this script
        ###hydra2 = hydra.hydra_client("192.168.10.3", 5000, 2, True)
        # We put the IP of the machine running the UE
        ###ue2 = xmlrpclib.ServerProxy("http://192.168.20.3:8080")
        # We put the IP of the machine running the client (this is for the client updates)
        ###client2 = xmlrpclib.ServerProxy("http://192.168.10.3:8080")

        ###if hydra2.check_connection(3) == "":
            ###print("hydra2 could not connect to server")
            ###sys.exit(1)
        ###slice2 = Slice(2, hydra2, client2, ue2)
        ###slice2.allocate_tx(1.43e9 + 300e3, 200e3, 0.08)
        ###slice2.allocate_rx(1.43e9 + 3e6 + 300e3, 200e3)
