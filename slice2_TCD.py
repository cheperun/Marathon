...
def create(slices,result,mf):
    if mf==0 or mf==1:#Add or Modify slice (When init mf=0)
        ...
        freqtx=2.43e9 #Canviar pel WINS_5G a 2.43e9
        freqrx=2.43e9 + 3e6 #Canviar pel WINS_5G a 2.43e9 +3e6
        ...
        #   slice3.allocate_rx(freqrx + 500e3, 200e3)
            ...
            #    slice_vue = vue3
            #else:
            #    print("Nep_vues does not exist")

            slice_vue =vue1 #to be modified
        ...
        if s == "shared":
            #if(contshared==3):
            #   si la slice que esborro està a hydra 2, la que hi ha a hydra1 ha de moure's a hydra2 **************
            contshared= contshared-1 #S'ha de decrementar quan s'esborra la slice !!!
            print "Decrease contshared to"
            print contshared
        else:
            #if(contnonshared==2):
            #   si la slice que esborro està a hydra1, la que hi ha a hydra2 s'ha de moure a hydra1 **************
            contnonshared=contnonshared-1 #S'ha de decrementar quan s'esborra la slice !!!
